<template>
  <div class="deposit">
      <div class="modal w-full h-full col-flex items-center gap-y-9 p-7">
          ...
          <qrcode-vue value="ulimhunyieagbama@gmail.com" level="H" size="230"></qrcode-vue>
          <div class="address row-flex relative w-full h-full items-center">
              <p class="text-sm text-left pl-2">TXoyJrBLgXBJYcrA9mVrXoTWyDQ1oVrKux</p>
              <button class="copy absolute right-1 bottom-1 text-white text-center" @click="copyAddress"><p v-if="addressCopied === false">Copy</p><p v-if="addressCopied === true">Copied</p></button>
          </div>
      </div>
  </div>
</template>
